...
layout (location = 2) in float in_bitangent_sign;
layout (location = 3) in vec2 in_tex_coord;

vec3 ComputeNormal(vec3 n, vec3 t, float s, sampler2D normal_map, vec2 tex_coord) {
    vec3 b = normalize(cross(n, t))*s;

    mat3 to_view_space = mat3(t, b, n);

    vec3 normal_color = texture(normal_map, tex_coord).xyz;
    vec3 tangent_space_normal = 2.0f*normal_color - 1.0f;
    vec3 view_space_normal = to_view_space*tangent_space_normal;

    return normalize(view_space_normal);
}
...
void main() {
    ...
    vec3 normal = ComputeNormal(n, t, in_bitangent_sign, normal_map, in_tex_coord);
