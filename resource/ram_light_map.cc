...
void LoadLightMap(LightMapId const &light_map_id, ResourceTable const &table,
                  VulkanContext *context, LightMap *light_map) {
    BOOST_LOG_TRIVIAL(info) << "LoadLightMap(): ID=[" << light_map_id << "].";
...
LightMap *LightMapRamTransfer::Load(LightMapId const &id, ResourceTable const &resource_table,
                                    VulkanContext *context) {
    LightMap *upload_result = cache_.Upload(
        ...
        [context, &resource_table](LightMapId id, uint64_t /*old_object_size*/,
                                   uint64_t /*new_object_size*/, LightMap *light_map) {
            LoadLightMap(id, resource_table, context, light_map);
        });
